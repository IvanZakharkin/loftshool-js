...
function findAllPSiblings(where) {
  const result = [];
  let element = where.firstElementChild;
...
function deleteTextNodesRecursive(where) {
  const getTextNodes = (node) => {
    return Array.from(node.childNodes).reduce((acc, el) => {
      let newAcc = acc;
  ...
  getTextNodes(where).forEach((node) => node.remove());
}
...
function collectDOMStat(root) {
  ...
  const collectNodeStat = (node) => {
    return Array.from(node.childNodes).reduce((acc, childeNode) => {
      if (childeNode.childNodes.length) {
      ...
      if (childeNode.nodeType === Node.TEXT_NODE) {
        ...
        if (!acc.tags[childeNode.tagName]) {
          acc.tags[childeNode.tagName] = 0;
        }

        acc.tags[childeNode.tagName] += 1;
        childeNode.classList.forEach((el) => {
          if (!acc.classes[el]) {
            acc.classes[el] = 0;
          }
